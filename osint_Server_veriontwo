...
type OSINTAgent struct {
	...
	Token        string    `json:"token"` // Added for per-agent authentication
	mutex        sync.Mutex
}
...
func (rl *RateLimiter) Allow(addr string) bool {
	...
	now := time.Now()
	if last, ok := rl.clients[addr]; ok && now.Sub(last) < rl.limit {
		return false
	}
	rl.clients[addr] = now
	return true
...
func NewOSINTControlServer(configPath string) (*OSINTControlServer, error) {
	...
	server := &OSINTControlServer{
		...
		running:     false,
		logger:      logger,
		rateLimiter: NewRateLimiter(1 * time.Second), // 1 req/sec per IP
...
func (s *OSINTControlServer) setupTLS() error {
	...
	caCert, err := ioutil.ReadFile(s.config.Server.SSL.CAPath)
	if err != nil {
...
func (s *OSINTControlServer) encryptState(data []byte) (string, error) {
	key := []byte(os.Getenv("STATE_ENCRYPTION_KEY"))
	if len(key) == 0 {
		return "", fmt.Errorf("STATE_ENCRYPTION_KEY not set")
	}
...
func (s *OSINTControlServer) decryptState(encrypted string) ([]byte, error) {
	key := []byte(os.Getenv("STATE_ENCRYPTION_KEY"))
...
func (s *OSINTControlServer) saveState() error {
	...
	encrypted, err := s.encryptState(data)
	if err != nil {
		return fmt.Errorf("failed to encrypt state: %w", err)
	}
	if err := ioutil.WriteFile(statePath, []byte(encrypted), 0600); err != nil {
...
func (s *OSINTControlServer) loadState() error {
	...
	decrypted, err := s.decryptState(string(data))
	if err != nil {
		...
		return fmt.Errorf("failed to decrypt server state: %w", err)
	}
...
func (s *OSINTControlServer) checkAgentConnection(agent *OSINTAgent) bool {
	...
	identifyCmd := CommandRequest{
		Command: "identify",
		Args:    map[string]interface{}{"token": agent.Token},
	}
...
func (s *OSINTControlServer) handleRegisterAgent(args map[string]interface{}) CommandResponse {
	...
	if _, err := rand.Read(tokenBytes); err != nil {
		return CommandResponse{Status: "error", Message: "Failed to generate agent token"}
	}
	...
	for id, job := range s.jobs {
		if filters != nil {
			...
			for key, value := range filters {
				switch key {
				case "agent_id":
					if v, ok := value.(string); !ok || job.AgentID != v {
						match = false
					}
				case "status":
					if v, ok := value.(string); !ok || job.Status != v {
						match = false
					}
				case "tool":
					if v, ok := value.(string); !ok || job.Tool != v {
						match = false
					}
				}
			}
...
func (s *OSINTControlServer) handleClient(conn net.Conn) {
	...
	if !s.rateLimiter.Allow(addr) {
		s.logger.Printf("Rate limit exceeded for %s", addr)
		return
	}
	...
	if !ok {
		s.logger.Printf("Connection is not TLS: %v", addr)
		return
	}
	...
	if len(state.PeerCertificates) == 0 {
		s.logger.Printf("Client did not provide certificate: %v", addr)
		return
	}
	cert := state.PeerCertificates[0]
	if cert.Subject.CommonName != "OSINT-Client" && cert.Subject.CommonName != "Flask-Server" {
		s.logger.Printf("Invalid client certificate CN: %s", cert.Subject.CommonName)
		return
	}

	s.logger.Printf("Client connected: %s", addr)
	...
	if cmd.Command == "" || len(cmd.Command) > 50 {
		response := CommandResponse{Status: "error", Message: "Invalid or missing command"}
		json.NewEncoder(conn).Encode(response)
		return
	}
	if cmd.Args == nil {
...
func main() {
	...
	if os.Getenv("STATE_ENCRYPTION_KEY") == "" {
		server.logger.Fatalf("STATE_ENCRYPTION_KEY environment variable must be set")
	}
